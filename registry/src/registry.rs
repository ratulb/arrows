...
impl Arrows {
    ...
    pub(crate) fn remove(&mut self, identity: u64) -> Option<Rc<RefCell<Box<dyn Actor>>>> {
...
pub fn register(
    ...
    println!("I am getting called alright {} ", addr);
    ...
    let existing_actor = CTX.write().unwrap().arrows.remove(addr);
    if let Some(existing_actor) = existing_actor {
        let mut existing_actor = existing_actor.borrow_mut();
        //Shutdown msg
        let _reply = existing_actor.receive(Msg::Blank);

        CTX.write().unwrap().storage.delete_actor(&identity);
    }
    let new_actor: Box<dyn Actor> = builder.build();
    let builder_def = serde_json::to_string(&builder as &dyn ActorBuilder)?;
    println!("Guru the type");
    use common::type_of;
    type_of(&new_actor);
    CTX.write()
        .unwrap()
