...
pub(crate) const ACTORS: &str =
    "CREATE TABLE IF NOT EXISTS actors (actor_id TEXT PRIMARY KEY, addr BLOB DEFAULT NULL, build_def TEXT, state BLOB DEFAULT NULL)";
...
pub(crate) const BUILD_DEF_INSERT: &str =
    "INSERT INTO actors (actor_id, addr, build_def) VALUES (:actor_id, :addr, :build_def)";
...
pub(crate) const BUILD_DEF: &str = "SELECT addr, build_def FROM actors WHERE actor_id = ?";
